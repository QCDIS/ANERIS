...
def do_work(input_file, output_folder, meta_file, cfg):
    ...
    with open(os.path.join(output_folder, "invasive_checker.log"), "w", encoding="utf8") as f:
    ...
    meta_df          = pd.read_csv(meta_file, sep=cfg.get('SEP')) 
    ...
    os.makedirs(os.path.join(output_folder, 'tests', 'output'), exist_ok=True)

    worms_df_unpivot.to_csv(os.path.join(output_folder, 'tests', 'output', 'unpivot.csv'), index=False)
    log.info('  -Looping through rows...') 
    wrims_df = worms_df_unpivot.apply(lambda row: process_input_row(row), axis=1)
    wrims_df.to_csv(os.path.join(output_folder, 'tests', 'output', 'wrims_df.csv'), index=False)

    # Clean up table
    wrims_df = wrims_df.drop('AccessionNumber', axis=1)
    wrims_df_unique = wrims_df[['classification', 'Aphia_ID']].drop_duplicates()
    ...
    aphia_df = pd.merge(worms_df, wrims_df_unique, how="left", on='classification')  # Use this to avoid large file sizes
    ...
    wrims_df.to_csv(filepath, sep=cfg.get('SEP'), index=False)
